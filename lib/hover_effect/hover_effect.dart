...
class FHoverCard extends StatefulWidget {
  ...
  @override
  FHoverCardState createState() => FHoverCardState();
}

class FHoverCardState extends State<FHoverCard> with SingleTickerProviderStateMixin {
  double localX = 0;
  double localY = 0;
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (_, dimens) {
        ...
        Widget body=Transform(
          ...
          child: Container(
            ...
            child: GestureDetector(
              ...
              onPanDown: (_){
                if(mounted)widget.onHover();
              },
              onPanEnd: (_) {
                //print('[hover effect] pan end');
                if(mounted){
                  widget.onHoverCancel();
                  setState(() {
                    isHover = true;
                    defaultPosition = false;
                  });
        ...
        /// disable gesture and hover effect
        if(!widget.enabled)body=Transform(
          transform: Matrix4.identity()
            ..setEntry(3, 2, 0.001)
            ..rotateX(defaultPosition ? 0 : (0.3 * (percentageY / 50) + -0.3))
            ..rotateY(defaultPosition ? 0 : (-0.3 * (percentageX / 50) + 0.3)),
